...
var dashboard$1 = {
    ...
    data: function data(){
        return {
            ...
            command: 'foreColor',
            colors: [
            	'rgba(0,0,0,1)','rgba(255,255,255,1)','rgba(96,125,139,1)','rgba(45,140,240,1)','rgba(25,190,107,1)','rgba(255,153,0,1)','rgba(237,64,20,1)','rgba(0,181,255,1)',
                'rgba(25,201,25,1)','rgba(249,227,28,1)','rgba(234,26,26,1)','rgba(155,29,234,1)','rgba(0,194,177,1)','rgba(172,122,51,1)',
                'rgba(29,53,234,1)','rgba(139,195,74,1)','rgba(241,107,98,1)','rgba(234,76,163,1)','rgba(13,148,170,1)','rgba(254,189,121,1)',
                'rgba(93,64,55,1)','rgba(0,188,212,1)','rgba(240,98,146,1)','rgba(205,220,57,1)'
            ]
        }
    },
    methods: {
...
var selectImage={
    ...
    i18n: 'selectImage',
    handler: function handler(editor) {
        editor.execCommand('selectImage',this.config);
    },
    config:{
    	editer:null,
    	...
    	refresh:function(){
    		if(null!=this.editer){
    			this.editer.toggleDashboard();
    			this.editer.$emit('change', this.editer.$refs.content.innerHTML,this.editer.fieldname);
    		}
    	}
    }
};
...
var template$8 = "<div> <button type=\"button\" @click=\"$parent.execCommand('bold')\"><b>{{$parent.locale[\"bold\"]}}</b></button> <button type=\"button\" @click=\"$parent.execCommand('italic')\"><i>{{$parent.locale[\"italic\"]}}</i></button> <button type=\"button\" @click=\"$parent.execCommand('underline')\"><u>{{$parent.locale[\"underline\"]}}</u></button> <button type=\"button\" @click=\"$parent.execCommand('strikeThrough')\"><strike>{{$parent.locale[\"strike through\"]}}</strike></button> <button type=\"button\" @click=\"$parent.execCommand('subscript')\"><sub>{{$parent.locale[\"subscript\"]}}</sub></button> <button type=\"button\" @click=\"$parent.execCommand('superscript')\"><sup>{{$parent.locale[\"superscript\"]}}</sup></button> </div> ";
...
RangeHandler.prototype.execCommand = function execCommand (command, arg) {
    ...
    switch (command) {
        ...
        case Command.SELECT_IMAGE:{
        	if(arg&&arg.selectMethod){
            	arg.selectMethod((function(src){
            		if(src){
                    	document.execCommand(Command.INSERT_IMAGE,false, src);
                    	arg.refresh();
            		}
            	}).bind(this));
        	}else{
        		document.execCommand(Command.INSERT_IMAGE);
        	}
        	break
        }
...
var editor = {
    ...
    methods: {
        ...
        execCommand: function execCommand(command, arg){
            ...
            if (this.range) {
            	if(command==Command.SELECT_IMAGE){
            		arg.editer=this;
            	}
                new RangeHandler(this.range).execCommand(command, arg);
            }
            this.toggleDashboard();
            this.$emit('change', this.$refs.content.innerHTML,this.fieldname);
        },
    ...
    mounted: function mounted(){
        ...
        content.addEventListener('keyup', function () {
            this$1.$emit('change', content.innerHTML,this$1.fieldname);
